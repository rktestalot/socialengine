...
class CommentTest < ActiveSupport::TestCase
  ...
  def test_should_notify_previous_anonymous_commenter
    configatron.allow_anonymous_commenting = true
    users(:quentin).comments.create!(:comment => 'foo', :author_email => 'foo@foo.com', :author_ip => '123.123.123', :recipient => users(:quentin))
    users(:quentin).comments.create!(:comment => 'bar', :author_email => 'foo@foo.com', :author_ip => '123.123.123', :recipient => users(:quentin))    
  ...
  def test_should_not_notify_previous_anonymous_commenter_if_self
    ...
    comment = users(:quentin).comments.create!(:comment => 'bar', :author_email => 'foo@foo.com', :author_ip => '123.123.123', :recipient => users(:quentin))        
  ...
  def test_should_not_notify_previous_anonymous_commenter_if_notify_by_email_is_false
    configatron.allow_anonymous_commenting = true
    users(:quentin).comments.create!(:comment => 'foo', :author_email => 'foo@foo.com', :author_ip => '123.123.123', :recipient => users(:quentin), :notify_by_email => false)
